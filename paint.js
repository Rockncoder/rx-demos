(function (window, document, Rx) {
  ...
  ['mousemove', 'mouseup', 'mousedown', 'click'].forEach( (name) => {
  ...
  function initialize() {
    ...
    var mouseDiffs = mouseMoves.zip(mouseMoves.skip(1),  (x, y) => {
    ...
    var paint = mouseButton.flatMapLatest((down) => {
        return down ? mouseDiffs : Rx.Observable.never()
      })
      .combineLatest(colorValues, (pos, color) => {
